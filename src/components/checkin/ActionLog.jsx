...
const ActionLog = ({ logs }) => {
    return (
        <div className="glass-panel" style={{ padding: '16px', marginTop: '16px', flex: 1, minHeight: '200px', display: 'flex', flexDirection: 'column' }}>
            <h3 style={{ margin: '0 0 12px 0', fontSize: '1rem', color: 'var(--color-text-secondary)' }}>受付ログ</h3>
            ...
            <div style={{ overflowY: 'auto', flex: 1, paddingRight: '4px' }}>
                {logs.map((log, i) => (
                    <div key={i} style={{
                        ...
                        borderRadius: '8px',
                        borderLeft: `3px solid ${log.result?.includes('check') || log.result?.includes('success') ? 'var(--color-success)' : (log.result === 'undo' ? 'var(--color-warning)' : 'var(--color-danger)')}`,
                        display: 'flex',
                        justifyContent: 'space-between',
                        alignItems: 'center'
                    }}>
                        <div>
                            <div style={{ fontWeight: 600, fontSize: '0.9rem' }}>{log.name || log.id || '不明'}</div>
